 ...
 *
 * A dotted prefix is a *pair*, not a decimal: `2.1-deseq2` is item 1 of branch
 * 2. That form exists because a nested folder tree reads much better on GitHub
 * when the children carry their parent's number, and someone who writes `01-`
 * instead gets the same result.
 *
 * The pair is folded into `major + minor / 1000` rather than read with
 * `Number()`, because `Number("2.10")` is 2.1 and would sort item 10 *before*
 * item 2 — which is wrong the moment a branch grows past nine items.
 */
export function parseName(filename: string): { order: number; slug: string } {
  ...
  const match =
    /^(\d{1,4})(?:\.(\d{1,4}))?[-_\s]+(.*)$/.exec(base) ??
    /^(\d{1,4})()[-_.\s]+(.*)$/.exec(base);

  if (match && match[3]) {
    const major = Number(match[1]);
    const minor = match[2] ? Number(match[2]) : 0;
    return { order: major + minor / 1000, slug: slugify(match[3]) };
  }
